...
// is a repo-root file, deliberately, because that is where someone browsing the
// project looks for it. So this is the one test that will not run from a
// module-only checkout - which is fine, because the thing it guards is a
// property of the repository rather than of the module.
func TestEmbeddedEnvMatchesShipped(t *testing.T) {
	...
	if string(shipped) != string(defaultEnv) {
		t.Error("embedded env_example.txt drifted from the repo-root .env-example")
	}
}
