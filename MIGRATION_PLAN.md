...
| E4 | TUI | `tui.py` (209 lines) | questionary/rich menu | `internal/cli/tui.go` | **VERIFIED** | `tui_test.go` (7 tests). The real stdin prompter was unreachable through Execute until it was given the command's input; the four pre-existing tests all inject a fake, so the production path had never run. |
...
| S7 | Notifier | `services/notifier.py` (98) | `internal/services/notifier` | **VERIFIED** | `notifier_test.go`: 4 pre-existing (states, gating, dangling coverage, clean tree) + `TestAFailedSendDoesNotConsumeTheCadence` (mutation-checked), `TestADeliveredNotificationMarksAndThenGatesTheCadence`, `TestTheCacheStampParsesEveryFormatItWasEverWrittenIn`, `TestALongDanglingListSaysHowManyItLeftOut`. Backend stubbed on PATH. |
...
| S10 | Importer | `services/importer.py` (270) | `internal/services/importer` | **VERIFIED** | `importer_test.go`: 3 pre-existing (parser, provider inference, expanduser) + `TestRunAdoptsAnOutsideKeyWithTightPermissions`, `TestDryRunWritesNothingAndAdoptsNothing`, `TestAdoptNeverOverwritesAKeyAlreadyInTheTree`, `TestAKeyAlreadyUnderItsProfileIsReferencedNotCopied`, `TestAnIncludeCycleTerminatesAndCollectsEveryHost`, `TestDuplicateHostBlocksKeepTheFirstAndAreCountedOnce` (caught a live bug: the summary counted parsed hosts, not imported ones; mutation-checked), `TestRunRejectsAMissingConfig` |
...
| S12 | Agent (ssh-add) | `services/agent.py` (26) | `internal/services/agent` | **VERIFIED** | `agent_test.go`: `TestLoadSharedDedupAndMissingSkip` (pre-existing) + `TestSSHAddArgsCarryTheKeychainFlagOnlyWhenAsked`, `TestADeclaredButUnwiredKeyIsNotLoadedIntoTheAgent` (pins D12) |
...
| D12 | `agent.Load` walks resolved hosts, not `KeyRefs` — unlike reconcile, validate and keyaudit | An agent holds keys for connections; a key no host uses serves none, and loading it would offer an extra identity to every server the user then talks to. Pinned by `TestADeclaredButUnwiredKeyIsNotLoadedIntoTheAgent` so it is not "fixed" into consistency. |
